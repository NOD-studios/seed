...
gulp.task('install-npm', (callback) => {
  ...
  return exec(`npm install ${options}`, (error, stdout, stderror) => {
    console.log(stdout);
...
gulp.task('install-jspm', (callback) => {
  let options = args.options || '';
  return exec(`jspm install ${options}`, (error, stdout, stderror) => {
    console.log(stdout);
    console.log(stderror);
    callback(error);
  });
...
gulp.task('install-composer', (callback) => {
  let options = args.options || '';
  return exec(`composer install ${options}`, (error, stdout, stderror) => {
    console.log(stdout);
    console.log(stderror);
    callback(error);
  });
